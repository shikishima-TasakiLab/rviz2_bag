...
void Recorder::record()
{
  ...
  if (storage_options_.snapshot_mode) {
    srv_snapshot_ = nh_->create_service<rosbag2_interfaces::srv::Snapshot>(
      "~/snapshot",
      [this](
  ...
  split_event_pub_ =
    nh_->create_publisher<rosbag2_interfaces::msg::WriteSplitEvent>("events/write_split", 1);
...
{
  auto all_topics_and_types = nh_->get_topic_names_and_types();
  TopicFilter topic_filter{record_options_, nh_->get_node_graph_interface()};
...
{
  auto subscription = nh_->create_generic_subscription(
    topic_name,
    topic_type,
    qos,
    [this, topic_name, topic_type](std::shared_ptr<rclcpp::SerializedMessage> message) {
      if (!paused_.load()) {
        writer_->write(message, topic_name, topic_type, nh_->get_clock()->now());
      }
    });
  return subscription;
}
...
std::string Recorder::serialized_offered_qos_profiles_for_topic(const std::string & topic_name)
{
  ...
  auto endpoints = nh_->get_publishers_info_by_topic(topic_name);
...
rclcpp::QoS Recorder::subscription_qos_for_topic(const std::string & topic_name) const
{
  ...
  return Rosbag2QoS::adapt_request_to_offers(
    topic_name, nh_->get_publishers_info_by_topic(topic_name));
}
...
void Recorder::warn_if_new_qos_for_subscribed_topic(const std::string & topic_name)
{
  ...
  auto publishers_info = nh_->get_publishers_info_by_topic(topic_name);
